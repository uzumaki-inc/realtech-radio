# #1 音声認識サービスの落とし穴｜便利さの裏で何を許可してる？

## 💡 エピソード概要

キーボードを打たずに声で文章入力——一度慣れると戻れない快適さ。でも、そのアプリに「何を許可しているか」を考えたことはありますか？

最新の音声入力サービスに感動した工藤が、ふと中身を覗いてみたら…。macOSのアクセシビリティ許可に潜む落とし穴を、非エンジニアにもわかるように紐解きます。

- 便利な音声入力ツールの裏側で、どんな権限を与えているのか
- 「データ保持ゼロ」といった安心表記を鵜呑みにしない、信頼性の見極め方（企業規模・第三者認証・オプトイン）
- 結局いま何を選べば無難か、という現実的な答え
- “音声入力ジプシー”工藤の遍歴：Super Whisper → Aqua Voice → Typeless

## 🔗 リンク

📝 もっと詳しく（note）：[文系リスナーと紐解く「リアルテックラジオ」#1｜UZUMAKI](https://note.com/uzumaki_inc/n/nd4d1f588acee)

🔗 登場：[Super Whisper](https://superwhisper.com) / [Aqua Voice](https://withaqua.com) / [Typeless](https://typeless.app)

## 🎙 クレジット

出演：工藤（株式会社UZUMAKI 代表取締役・[X](https://x.com/ToraDady)）／ はっしー（[X](https://x.com/chrhsmt)）／ だいそん（[X](https://x.com/daidai3110)）

制作：[株式会社UZUMAKI](https://uzumaki-inc.jp)

## 📻 番組概要

エンジニアが普段何を考え、どんなことにワクワクしているのかを、非エンジニアの視点で紐解いていくラジオ。文系リスナーが社内のエンジニアの会話を理解できるように翻訳しつつ、エンジニアが共感できる学びのきっかけになるコンテンツです。
